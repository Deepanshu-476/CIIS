...
export default function LeadTypes(){
  ...
  useEffect(() => {
    let active = true;
    api.get('/crm/lead-types', { cache: false }).then(({data}) => {
      if (active) setItems(data.items.map(item => ({ ...item, id: item._id })));
    }).catch(err => { if (active) setError(err.response?.data?.message || 'Could not load lead types. Please reload to try again.'); })
  ...
  const save=async e=>{
    ...
    try {
      const response=modal?.item ? await api.put(`/crm/lead-types/${modal.item.id}`,payload) : await api.post('/crm/lead-types',payload);
      ...
      setModal(null);
    } catch(err) { setError(err.response?.data?.message || 'Could not save lead type. Please try again.'); }
    finally {inFlight.current=false;setBusy(false);}
  };
  return <main className="ls-root">
    <header className="ls-page-header"><h1 className="ls-page-title">Lead Type</h1><div className="ls-breadcrumb"><span>Dashboard</span><span className="separator">›</span><span className="active">Lead Type</span></div></header>
    {error && !modal && <p role="alert" className="ls-empty-row">{error}</p>}
    <section className="ls-card"><header className="ls-card-header flex-between"><h2 className="ls-card-title">Lead Type</h2><button type="button" className="ls-btn-add" disabled={loading || busy} onClick={()=>{setError('');setModal({})}}><FiPlusCircle size={15}/> Add Lead Type</button></header>
      <div className="ls-table-controls"><div className="ls-entries-selector"><select value={limit} onChange={e=>{setLimit(Number(e.target.value));setPage(1)}}><option>10</option><option>25</option><option>50</option></select><span>entries per page</span></div><div className="ls-search-box"><label>Search:</label><input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}/></div></div>
      <div className="ls-table-responsive"><table className="ls-table"><thead><tr><th style={{width:80}}>SL NO.</th><th>LEAD TYPE</th><th style={{width:120}}>STATUS</th><th style={{width:100}}>ACTION</th></tr></thead><tbody>{visible.map((item,index)=><tr key={item.id}><td className="ls-sl-col">{String(start+index+1).padStart(3,'0')}</td><td>{item.name}</td><td><span className={`lt-status-pill ${item.status === 'Active' ? 'is-active' : 'is-inactive'}`}><span aria-hidden="true" />{item.status}</span></td><td><div className="ls-action-btns"><button type="button" className="ls-btn-edit" disabled={busy} onClick={()=>{setError('');setModal({item})}}><FiEdit2 size={12}/></button></div></td></tr>)}{!visible.length&&<tr><td colSpan="4" className="ls-empty-row">{loading ? 'Loading lead types...' : 'No lead types found.'}</td></tr>}</tbody></table></div>
