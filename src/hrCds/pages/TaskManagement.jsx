...
const UserCreateTask = () => {
  ...
  const fetchAssignedToMeTasks = useCallback(async () => {
    ...
    try {
      ...
      params.set('page', String(assignedPagination.page));
      params.set('limit', String(assignedPagination.limit));
      ...
      if (res.data && (res.data.success || res.data.groupedTasks || Array.isArray(res.data))) {
        ...
        setAssignedPagination(prev => ({ ...prev, total: 0, pages: 1, hasNext: false, hasPrev: false }));
      }
  ...
  }, [authError, userId, enrichAssignedTasks, extractTasksFromResponse, groupTasksByDate, calculateAssignedStatsFromTasks, tagTasksWithSource, buildTaskQueryParams, assignedPagination.page, assignedPagination.limit]);
  ...
  const fetchMyTasks = useCallback(async () => {
    ...
    try {
      ...
      const params = new URLSearchParams(query);
      params.set('page', String(personalPagination.page));
      params.set('limit', String(personalPagination.limit));
      const url = `/tasks/self?${params.toString()}`;
      void 0;
      ...
      setPersonalPagination(prev => ({
        ...prev,
        ...(res.data?.pagination || {}),
        total: res.data?.pagination?.total ?? res.data?.total ?? selfTasks.length,
        pages: res.data?.pagination?.pages ?? 1
      }));

    } catch (err) {
      console.error('Error fetching my tasks:', err);
      setPersonalPagination(prev => ({ ...prev, total: 0, pages: 1, hasNext: false, hasPrev: false }));
      if (err.response?.status === 401) {
        setAuthError(true);
        showSnackbar('Session expired. Please log in again.', 'error');
      } else {
  ...
  }, [authError, userId, calculateStatsFromTasks, extractTasksFromResponse, groupTasksByDate, tagTasksWithSource, buildUserTaskQueryParams, getUserTaskApiPeriod, personalPagination.page, personalPagination.limit]);
  ...
  const handleViewModeChange = (mode) => {
    ...
    if (mode === 'all') {
      ...
      setPersonalPagination(prev => ({ ...prev, page: 1 }));
    } else if (mode === 'assigned') {
      setAssignedPagination(prev => ({ ...prev, page: 1 }));
    }
  };
  ...
  }, [userId, authError, pageLoading, fetchOverdueTasks]);

  useEffect(() => {
    if (!userId || authError || pageLoading) return;

    const loadActiveTab = async () => {
      if (taskViewMode === 'all' && !loadedTabs.all) {
        await fetchAllTasks();
        setLoadedTabs(prev => ({ ...prev, all: true }));
      } else if (taskViewMode === 'self' && !loadedTabs.self) {
        await fetchMyTasks();
        setLoadedTabs(prev => ({ ...prev, self: true }));
      } else if (taskViewMode === 'assigned' && !loadedTabs.assigned) {
        await fetchAssignedToMeTasks();
        setLoadedTabs(prev => ({ ...prev, assigned: true }));
      } else if (taskViewMode === 'client' && !loadedTabs.client) {
        await fetchClientTasks();
        setLoadedTabs(prev => ({ ...prev, client: true }));
      } else if (taskViewMode === 'project' && !loadedTabs.project) {
        await fetchProjectTasks();
        setLoadedTabs(prev => ({ ...prev, project: true }));
      }
    };

    loadActiveTab();
  }, [userId, authError, pageLoading, taskViewMode, loadedTabs.all, loadedTabs.self, loadedTabs.assigned, loadedTabs.client, loadedTabs.project, fetchAllTasks, fetchMyTasks, fetchAssignedToMeTasks, fetchClientTasks, fetchProjectTasks]);
    ...
    <div className="user-create-task-container">
      ...
      <div className="user-create-task-paper">
        ...
        <div className="user-create-task-paper-content">
          ...
          {taskViewMode === 'self' && !loading && personalPagination.total > personalPagination.limit && (
            <div className="user-create-task-pagination">
              <button
                className="user-create-task-button user-create-task-button-outlined"
                disabled={!personalPagination.hasPrev}
                onClick={() => setPersonalPagination(prev => ({ ...prev, page: Math.max(1, prev.page - 1) }))}
              >
                Previous
              </button>
              <div className="user-create-task-pagination-info">
                Page {personalPagination.page} of {personalPagination.pages} • {personalPagination.total} tasks
              </div>
              <button
                className="user-create-task-button user-create-task-button-outlined"
                disabled={!personalPagination.hasNext}
                onClick={() => setPersonalPagination(prev => ({ ...prev, page: prev.page + 1 }))}
              >
                Next
              </button>
            </div>
          ...
          {taskViewMode === 'assigned' && !loadingAssigned && assignedPagination.total > assignedPagination.limit && (
            <div className="user-create-task-pagination">
              <button
                className="user-create-task-button user-create-task-button-outlined"
                disabled={!assignedPagination.hasPrev}
                onClick={() => setAssignedPagination(prev => ({ ...prev, page: Math.max(1, prev.page - 1) }))}
              >
                Previous
              </button>
              <div className="user-create-task-pagination-info">
                Page {assignedPagination.page} of {assignedPagination.pages} • {assignedPagination.total} tasks
              </div>
              <button
                className="user-create-task-button user-create-task-button-outlined"
                disabled={!assignedPagination.hasNext}
                onClick={() => setAssignedPagination(prev => ({ ...prev, page: prev.page + 1 }))}
              >
                Next
              </button>
            </div>
