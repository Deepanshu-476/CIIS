    ...
    const calculateTasksForAll = async () => {
      ...
      for (const client of clients) {
        try {
          ...
          const overdueTaskNames = tasks
            .filter(t => isClientTaskOverdue(t))
            .map(task => task.task || task.taskName || task.title || task.name || 'Untitled Task');
          const overdueTasksCount = overdueTaskNames.length;
                  ...
                  {overdueClients.map((client) => (
                    <div key={client._id} className="ClientManagement-overdue-client-item">
                      <div>
                        ...
                        <div className="ClientManagement-overdue-task-names">
                          {(client.overdueTaskNames || []).map((taskName, taskIndex) => (
                            <small key={`${client._id}-overdue-task-${taskIndex}`} className="ClientManagement-overdue-task-name">
                              • {taskName}
                            </small>
                          ))}
                        </div>
