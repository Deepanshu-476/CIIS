...
export default function SalaryStructure() {
  ...
  const [selected,setSelected]=useState(""), [pickerOpen,setPickerOpen]=useState(false), [editingId,setEditingId]=useState(""), [message,setMessage]=useState(null), [loading,setLoading]=useState(true), [saving,setSaving]=useState(false);
  ...
  const add=()=>{const component=masters.find(x=>x._id===selected);if(!component||used.has(selected))return;change("components",[...form.components,rowFor(component,form.components.length+1)]);setSelected("");setPickerOpen(false);};
  ...
  return <main className="salary-structure-page">
    ...
    <section className="ss-card">
      <div className="ss-card-title ss-between"><div><h2>Salary Components</h2><p>Add components and define calculation rules for this structure.</p></div><button className="ss-add-button" type="button" onClick={()=>setPickerOpen(true)}><FiPlus/>Add Component</button></div>
    ...
    {pickerOpen&&<div className="ss-modal-backdrop" onMouseDown={()=>setPickerOpen(false)}><div className="ss-modal" onMouseDown={e=>e.stopPropagation()}><div><h3>Add Salary Component</h3><p>Select a configured component to include in this structure.</p></div><label><span>Salary Component</span><select value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Select component</option>{masters.filter(x=>!used.has(x._id)).map(x=><option key={x._id} value={x._id}>{x.name} ({x.code})</option>)}</select></label><footer><button type="button" onClick={()=>setPickerOpen(false)}>Cancel</button><button type="button" className="primary" disabled={!selected} onClick={add}><FiPlus/>Add Component</button></footer></div></div>}
  </main>;
}
